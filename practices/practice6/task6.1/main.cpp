...
int main()
{
	...
	String s1("C++");
	std::println("Вывод: '{}', Размер: {}", s1.get_c(), s1.Size());
	s1[0] = 'A';
	std::println("Замена первого на A: '{}', Размер: {}", s1.get_c(), s1.Size());

	String s2 = std::move(s1);
	std::println("Перемещенный объект: '{}', Первый объект пуст? {}", s2.get_c(), s1.Size() == 0);
	return 0;
}